...
### 2026-03-27
- **feat**: Auto-detect and fix serial port permissions. Ports like `/dev/ttyACM0` often require `sudo chmod 666` on Linux — the UI now detects inaccessible ports after scanning, shows a "Fix Permissions" banner, and attempts to fix them via `pkexec` (graphical password prompt) or installs a permanent udev rule. Ports are marked "(no permission)" in dropdowns when not accessible. The wiggle endpoint also pre-checks permissions before connecting. Added: `backend/services/port_permissions.py`. Modified: `backend/api/setup.py`, `backend/api/system.py`, `backend/models/setup.py`, `backend/services/port_scanner.py`, `frontend/components/wizard/steps/ports-step.tsx`, `frontend/lib/services.ts`, `frontend/lib/wizard-types.ts`, `frontend/lib/api.ts`.
- **fix**: UI broken on Jetson Nano (Chromium 107) — sidebar steps rendered as solid black boxes, error panels were solid red with invisible text. Two root causes: (1) `oklch()` CSS color functions unsupported — replaced all theme variable values with hex equivalents. (2) `color-mix()` unsupported (requires Chrome 111) — Tailwind v4 uses `color-mix()` for opacity modifiers (e.g. `bg-primary/5`) and falls back to the full-opacity color when unsupported, causing `bg-primary/5` to render as solid black and `bg-destructive/5` as solid red. Added comprehensive `@supports not (color: color-mix(...))` fallback block with rgba() equivalents for all opacity-modified theme colors used in the app. Modified: `frontend/app/globals.css`.
